...
(function () {
  ...
  // one Metropolis step on the blended density; a measurement pulls walkers
  // toward the core so the cloud collapses, then re-expands into the new shape
  const UPDATE_FRAG = M.FRAG_HEADER + M.GLSL_NOISE + ORB_LIB + `
  uniform sampler2D uParts;
  uniform int uN;
  uniform float uShape, uStep, uT, uCollapse;
  void main() {
    ...
    float step = uStep * (1.0 + uCollapse * 1.6);   // agitate during a measurement
    vec3 prop = pos + jit * 2.0 * step;

    float pOld = orbDensity(uN, uShape, pos);
    float pNew = orbDensity(uN, uShape, prop);
    if (hash12(seed + 13.0) * (pOld + 1e-9) < pNew) pos = prop;

    if (uCollapse > 0.001) pos = mix(pos, pos * 0.12, uCollapse * 0.16); // implode
  ...
  void main() {
    ...
    float shimmer = 0.72 + 0.28 * sin(az * 12.566 - uShimmer); // angular current
    float glow = (0.35 + band * 2.4 * uColEnergy) * shimmer;

    float hue = psi >= 0.0 ? uKeyHue : uKeyHue + 0.5;
    vec3 c = pal(hue, vec3(0.5), vec3(0.5), vec3(1.0), vec3(0.0, 0.33, 0.67));
    c += band * band * vec3(0.55, 0.7, 1.0) * 0.7;  // hot frequencies whiten
  ...
  M.registerScene({
    ...
    create(glc) {
      ...
      // a rare, earned "measurement": new shell + orientation, then a collapse
      function measure() {
        n = Math.min(4, Math.max(2, 2 + Math.round(register * 2)));
        tilt = 0.25 + Math.random() * 1.1;
      ...
      return {
        ...
        update(dt, audio, t) {
          ...
          // continuous structure: harmony grows/retracts the lobes
          shape = Math.max(0, Math.min(n - 1, richness * (n - 1) * 1.05));

          // motion every frame: spin on loudness, current shimmer
          ...
          // Metropolis iterations; loudness agitates the walk
          const step = (0.10 * n * n + 0.12) * (1.0 + f.level * 0.6);
          pUpdate.use().i('uN', n).f('uShape', shape).f('uStep', step).f('uCollapse', collapse);
